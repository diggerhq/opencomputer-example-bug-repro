...
A bug report and a public repository go in. A failing test, its output, the
file and line, and the cause come out.

One function. No custom tools. No credentials. The agent runs inside
OpenComputer's managed harness, a complete coding agent in its own VM; the
function decides which of the harness's tools the model gets for each step.
...
The function runs before every model step. Its `useTool` calls and return
value are recorded as `agent.rendered` before the model is called. Four
scenarios against one deployment, from the live event log.
...
One turn, eight model steps, 55 seconds. The test file is in the workspace.
...
Same deployment, same harness. Nothing was selected, so nothing existed for
the model to call.

## 3. A webhook, delivered twice
...
202  { "request": { "sessionId": "5b0420db-…" }, "duplicate": true }    # same key, same body, resent
...
The payload carries the repository, so the same function attaches the same
tools. A retrying upstream gets the original session back, not a second run.

## 4. The same session, after it suspended
...
Resuming 56d3d45e-…            # status was suspended; the VM comes back with its disk
...
The workspace is part of the session. So is the conversation.

One rule to know: the function sees the input that started the current
turn, not the conversation. A follow-up that omits the repository URL renders
scenario 2 and gets no tools for that turn, clone or no clone. Name the
repository in each request, as a webhook payload does, or carry it in
session data (`useSessionData()`; no public write API yet).

## Run it

Node 22 and an OpenComputer account.
...
npx opencomputer deploy --watch --create-project bug-repro   # builds, creates the project, deploys to development, redeploys on save
```

In a second terminal, run scenarios 1 and 2 as shown. For scenario 3, create
the webhook and post the payload. For scenario 4, take the session id from
`npx opencomputer session list` and use `session send`.

Point it at any public repository: paste the URL and the report into the
session text, or send them as `payload.repository`, `payload.path`, and
`payload.report`.

## Look inside

```bash
npx opencomputer sessions tail <session-id> --after 0 --no-follow --json   # the durable event log as NDJSON
```

`agent.rendered` carries `enabledTools`, the instructions hash, the
deployment id, provider turn, and state version. `tool.completed` carries
the shell output. `session.status_changed` shows `suspending` and
`suspended` about a second after a turn ends. The dashboard session
inspector shows the same records with the full instructions.

After a build, `opencomputer/agents/bug-repro/.opencomputer/runtime/.opencomputer/reactive.json`
is what the compiler extracted from the source: the tool list every render
is checked against, and nothing else this agent could reach.
...
fixture/src/invoice.js                   a billing module with two real bugs
fixture/test/invoice.test.js             the existing suite; passes with both bugs present
fixture/BUG-REPORTS.md                   the two reports as they arrive
DX-NOTES.md                              what was observed against the live platform
```

## Boundaries

- Public repositories only; the clone is unauthenticated. Credentials go
  through declared connections with the secret attached outside the VM; see
  the [Pull Request Reviewer](https://github.com/diggerhq/opencomputer-example-pr-review).
- The repository's own scripts run inside the session VM. Treat the code you
  point the agent at as untrusted. "Do not modify existing files" is an
  instruction, not an enforced boundary. The VM holds no credentials.
- Not selected here: subagents, skills, MCP servers.
